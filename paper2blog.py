...
# Try to import OpenAI, if not available, provide a helpful error message
try:
    from openai import OpenAI
    OPENAI_AVAILABLE = True
except ImportError:
    logger.warning("OpenAI package not installed. Please run 'pip install openai' to use this feature.")
    OPENAI_AVAILABLE = False
    OpenAI = None

# Try to import fitz (PyMuPDF) for PDF to PNG conversion
try:
    import fitz  # PyMuPDF
    PDF_CONVERSION_AVAILABLE = True
except ImportError as e:
    logger.warning(f"PyMuPDF not installed. PDF to PNG conversion will be skipped. Error: {e}")
...
def convert_pdf_to_png(pdf_path: str, output_dir: str) -> str | None:
    ...
    if not PDF_CONVERSION_AVAILABLE:
        logger.warning("PDF conversion skipped: PyMuPDF not installed.")
        return None
...
def generate_blog_post(
    ...
    if not OPENAI_AVAILABLE:
        logger.error("OpenAI package is required but not installed. Please run 'pip install openai'")
        return False
    ...
    try:
        ...
        if OpenAI is None:
            logger.error("OpenAI package is not available. Cannot generate blog post.")
            return False
        ...
        # model_to_use='Qwen/Qwen2.5-7B-Instruct' # 'Qwen/Qwen3-32B' 'Qwen/Qwen2.5-72B-Instruct'(图片引用差,输出中断)
        model_to_use = 'deepseek-ai/DeepSeek-V3.1' # 'MiniMax/MiniMax-M2'(图片引用最佳,文本流畅)  'deepseek-ai/DeepSeek-V3.1' 'deepseek-ai/DeepSeek-R1-0528' 'ZhipuAI/GLM-4.6'(输出中断)
        ...
        if not blog_content:
            logger.error("No blog content found in the response.")
            return False
